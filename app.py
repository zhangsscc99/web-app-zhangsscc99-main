...
'''
sender=''
@app.route('/readone',methods=['POST'])

def createone():
    sender=request.form['fsname']
    print(sender)
    doc2={
        "name":sender
    }

    db.exampleapp.insert_one(doc2)
...
def readone():
    ...
    docs = db.exampleapp.find({"name":sender})
    
    return render_template('readone.html',docs=docs)
...
def createone():
    ...
    db.exampleapp.insert_one(new_doc)
...
def create_post():
    ...
    hobby = request.form['fhobby']
    receiver =request.form['freceiver']
